...
Runner::Runner() {
  OATPP_COMPONENT(std::shared_ptr<
                ...
                router, Qualifiers::SERVICE_USER);
  ...
  docEndpoints.append(router->addController(
                std::make_shared<
                controller::UserController>())->getEndpoints());

  OATPP_COMPONENT(std::shared_ptr<
                oatpp::swagger::DocumentInfo>,
                documentInfo, Qualifiers::SERVICE_USER);
  OATPP_COMPONENT(std::shared_ptr<
                oatpp::swagger::Resources>, resources,
...
void Runner::run(std::list<std::thread>& acceptingThreads) {
  ...
  connectionHandler->addRequestInterceptor(std::make_shared<oatpp::web::server::interceptor::AllowOptionsGlobal>());
  connectionHandler->addResponseInterceptor(std::make_shared<oatpp::web::server::interceptor::AllowCorsGlobal>());
  connectionHandler->addResponseInterceptor(std::make_shared<rest_its::user::controller::CustomCorsMethodsInterceptor>());

  acceptingThreads.push_back(std::thread([router, connectionHandler]{
    OATPP_COMPONENT(std::shared_ptr<
                  oatpp::network::ServerConnectionProvider>,
                  connectionProvider, Qualifiers::SERVICE_USER);
    oatpp::network::Server server(connectionProvider, connectionHandler);
    OATPP_LOGI("user-service", "server is listening on port '%s'",
  ...
  acceptingThreads.push_back(std::thread([router, connectionHandler]{
    OATPP_COMPONENT(std::shared_ptr<
                  oatpp::network::ServerConnectionProvider>,
                  connectionProvider, Qualifiers::SERVICE_USER_VH);
    oatpp::network::Server server(connectionProvider, connectionHandler);
    OATPP_LOGI("user-service", "server is listening on virtual interface '%s'",
